...
module {
  ...
  func hashLabeled(l : Key, s : Hash) : Hash {
    h3("\13ic-hashtree-labeled", l, s);
  };
    ...
    switch (ki.next()) {
      ...
      case (?k) {
        switch (tree) {
          ...
          case (#leaf(_)) { #pruned(treeHash(tree)) };
          case (#subtree(s)) {
            switch (s) {
              case null { #empty };
              case (?t) {
  ...
  func revealLeaf(t : T, ki : Iter.Iter<Key>) : (Bool, Witness, Bool) {
    switch (t) {
      case (#fork(_)) { (true, revealMinKey(t), false) };
      case (#prefix(l)) {
  ...
  func revealLeft(t : T, p : Prefix, ki : Iter.Iter<Key>) : (Bool, Witness, Bool) {
    switch (t) {
      case (#fork(f)) {
        ...
        let w2 = if b2 { revealMinKey(f.right) } else {
          #pruned(hashT(f.right));
        };
        (b1, #fork(w1, w2), false);
      };
      case (#prefix(l)) {
        switch (l.rest) {
          case null {
            (false, #labeled(l.key, #pruned(labeledTreeHash(l.here))), true);
          };
          case (?t2) {
            let (b1, w1, b2) = revealT(t2, p, ki);
            let w0 = if b1 { #labeled(l.key, #pruned(labeledTreeHash(l.here))) } else {
              #pruned(l.labeled_hash);
            };
            (false, #fork(w0, w1), b2);
          };
        };
